...
#
# This one costs money: three refreshes, each a file pass plus synthesis for
# `engine/core` and every directory above it. It needs a fixture that is already
# pacted and a clean tree, because it restores with git.
#
#   ./incremental.sh
set -uo pipefail
...
[ -f .warlock/pacts.toml ] || { echo "the fixture is not pacted — run ./check.sh first"; exit 2; }
...
restore() { git checkout -- . 2>/dev/null; git clean -qfd; }
trap restore EXIT
